...
tof_cell  = zeros(200,100,20,3);%{Receiver, Transmitter, ERate, Num, }
aveSOS = zeros(20,3);%{ERate, Num}
steSOS = zeros(20,3);
leng = zeros(1,100);
t_size = 40.e-3;
t_num = 200;%トランスデューサ数
...
for i = 1:3
    for j = 2:2:20
        cd('\data\kwave\medium\2018_08_02_patternScatter');
        myfilename = sprintf('rEMCL%d & nEMCL%d',j,i);
        load(myfilename);
        param.source.point_map = cast(linspace(1,100,100),'int8');
        cd('\data\kwave\result\2018_08_02_patternScatter');
        dst_path = myfilename;
        ...
        cd('\result\2018_08_02_analyzePatternModel');
