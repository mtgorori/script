...
tof_cell  = zeros(200,100,400);%{Receiver, Transmitter, ERate, Num, }
aveSOS = zeros(1,400);%{ERate, Num}
steSOS = zeros(1,400);
aveSOS2 = zeros(1,400);%{ERate, Num}
steSOS2_0 = zeros(100,400);
steSOS2 = zeros(1,400);
leng = zeros(1,100);
leng2 = zeros(1,100);
t_size = 40.e-3;
...
for ii = 301:320
    cd('\data\kwave\medium\2018_08_28_realisticScatter');
    myfilename = sprintf('case%d',ii);
    load(myfilename);
    param.source.point_map = cast(linspace(1,100,100),'int8');
    cd('\data\kwave\result\2018_08_28_realisticScatter');
    dst_path = myfilename;
    ...
    cd('\result\2018_08_28_analyze400RealisticModels');
...
cd('\result\2018_08_28_analyze400RealisticModels');
myfilename = sprintf('2018_08_28_aveSOS&steSOS_301-320');
save(myfilename,'aveSOS','steSOS','tof_cell','aveSOS2','steSOS2');